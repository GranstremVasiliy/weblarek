...
Методы класса:
`validate(): { [K in keyof ICustomer]?: string }` - Проверяет корректность заполнения всех полей, необходимых для заказа (payment, address, email, phone). Возвращает объект с сообщениями об ошибках;
...
`bindHandlers(): void`-регистрация событий формы (выбор способа оплаты с генерацией form:change, и клик по кнопке "далее" с генерацией form:order-submit).
...
`form:change` - генерируется родительским классом Form при изменении текстовых полей и OrderForm при изменении кнопок оплаты;
...
`customer:updated` - Генерируется CustomerModel после каждого изменения данных. Запускает цикл валидации и рендера форм в Презентере.
